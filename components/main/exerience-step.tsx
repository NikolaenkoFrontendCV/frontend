...
export default function ExperienceStep({
  ...
  return (
    <div
      className={`relative flex w-full shrink-0 content-stretch items-center justify-center gap-10 overflow-visible px-10 py-2.5 max-sm:flex-col max-sm:items-start max-sm:gap-5 max-sm:px-5 max-sm:py-2.5 ${reverse ? "flex-row-reverse" : ""}`}
    >
      <AnimationText
        direction={reverse ? "rightToLeft" : "leftToRight"}
        className={`flex min-h-px w-full min-w-px flex-col content-stretch ${reverse ? "items-start" : "items-end"} justify-center max-sm:items-start`}
        data-node-id="2117:169"
      >
        <div
          className="flex shrink-0 flex-col content-stretch items-center justify-center gap-2.5 leading-0 whitespace-nowrap not-italic max-sm:items-start"
      ...
      <div className="top-4 -left-1.5 aspect-square min-w-3 rounded-full bg-white max-sm:absolute"></div>
      <AnimationText
